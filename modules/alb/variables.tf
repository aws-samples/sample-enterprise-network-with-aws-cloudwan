...
variable "subnet_ids" {
  ...
  validation {
    ...
    error_message = "At least 2 subnet IDs must be provided for ALB deployment across multiple AZs."
  }
}
...
variable "alb_access_logs_s3_bucket_force_destroy" {
  type        = bool
  default     = true # Changed to true for easier cleanup
...
variable "target_groups" {
  ...
  default     = {}
  description = "Map of target group configurations. Key is used as target group identifier."
}
...
variable "listener_rules" {
  type = map(object({
    ...
    conditions = list(object({
      field  = string # "host-header", "path-pattern", "http-request-method", etc.
      values = list(string)
    }))
  ...
  default     = {}
  description = "Map of listener rules for routing traffic to different target groups"
}
...
# Variables to replace cloudposse context functionality
variable "enabled" {
  type        = bool
  default     = true
...
# Missing lifecycle variables
variable "lifecycle_rule_enabled" {
  type        = bool
  default     = false
  description = "Enable lifecycle rule for S3 bucket"
}

variable "standard_transition_days" {
  type        = number
  default     = 30
  description = "Number of days to transition to Standard-IA storage class"
}
...
variable "noncurrent_version_transition_days" {
  type        = number
  default     = 30
  description = "Number of days to transition noncurrent versions"
}
